...
counts_miRNA <- data.frame(counts_miRNA %>% select((meta_data %>% filter(experimental_class_type %in% c(condition, control), `Assay Type` == 'miRNA-Seq'))$Run), row.names = counts_miRNA$X1)
...
```{r, message = F, warning = F}
# Create DESeqDataSet object
dds_miRNA <- get_DESeqDataSet_obj(counts_miRNA, ~ experimental_class_type, counts_threshold = 5)
